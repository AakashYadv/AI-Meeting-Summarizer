...
try:
    ...
    sendgrid = None  # If not installed, just fallback to SMTP


def _validate_emails(to_emails):
    """Ensure recipient list only has valid email addresses."""
    cleaned = []
...
def _send_via_sendgrid(to_emails, subject, body_markdown):
    """Send email using SendGrid API (if configured)."""
    ...
    mail = Mail(
        ...
        subject=subject,
        html_content=(
            f"<pre style='font-family: ui-monospace, SFMono-Regular, Menlo, "
            f"Consolas, monospace; white-space: pre-wrap'>{body_markdown}</pre>"
        ),
    )
    return sg.send(mail)
...
def _send_via_smtp(to_emails, subject, body_markdown):
    """Send email using SMTP (default fallback)."""
    ...
    msg["Subject"] = subject
    msg["From"] = username or os.getenv("SMTP_FROM_EMAIL", "no-reply@example.com")
...
def send_email(to_emails, subject, body_markdown):
    ...
    try:
        ...
        print("⚠️ SendGrid failed, falling back to SMTP:", str(e))
